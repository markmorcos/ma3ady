...
The web-app capability covers the authenticated + public-booking web surface at `app.ma3ady.com`, exported from the Expo Router source tree via React Native for Web. It ships from the same `app/` and `src/` as the mobile build; web-specific code lives behind `.web.ts(x)` platform-extension wrappers.
