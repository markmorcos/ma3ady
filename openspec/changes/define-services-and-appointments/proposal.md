...
- Required by the `implement-public-booking-flow`, `implement-admin-mobile-dashboard`, `implement-reschedule-and-cancel` and `implement-notifications-pipeline` changes.
- Finalizes the body of `compute_available_slots` from the `define-availability-rules` change (services + appointments now exist).
