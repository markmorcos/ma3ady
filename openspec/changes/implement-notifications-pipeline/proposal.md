...
Push is wired via the dispatcher interface but defaults to `mock` (in-app toasts) until a dev client ships in the `setup-compliance-and-launch` change.
...
- **ADDED** dispatcher modules in `supabase/functions/_shared/dispatchers/`:
  ...
  - `push.ts`: `MockPushDispatcher`, `ExpoPushDispatcher` (deferred to the `setup-compliance-and-launch` change)
- **ADDED** templates:
...
- `mock` mode is the default in dev/preview; production EAS profile assertion already enforces `real` (the `setup-monorepo-and-tooling` change).
