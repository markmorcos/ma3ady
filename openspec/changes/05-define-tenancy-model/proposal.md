...
- **ADDED** migration `001_tenancy.sql` introducing:
  - `tenants(id uuid pk, slug text unique not null, name text not null, timezone text not null, default_locale text not null, brand_color text, logo_url text, created_at, updated_at)`
...
- Required by changes 06, 07, 09, 10, 11.
- No user-facing UI yet — tenant switching UI lands in change 09.
