...
After Google OAuth lands (change 08), a signed-in user with no tenant memberships needs a path to either join an existing tenant (invitation) or create one. This change implements the "claim a slug, become owner" flow plus the tenant picker for users who belong to multiple tenants.
