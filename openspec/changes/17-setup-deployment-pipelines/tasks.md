...
- [ ] 17.1 Create `.github/workflows/ci.yml`:
  ...
  - Use Node 20 LTS
- [ ] 17.2 Create `.github/workflows/deploy-marketing.yml`:
...
- [ ] 17.3 Create `.github/workflows/deploy-tenant-landing.yml` mirroring marketing
- [ ] 17.4 Create `.github/workflows/deploy-supabase.yml`:
...
- [ ] 17.5 Create `.github/workflows/build-mobile.yml`:
...
- [ ] 17.6 Add to `eas.json` (created in change 01) the production-profile pre-build hook script that asserts `EXPO_PUBLIC_*_DISPATCHER === 'real'`
- [ ] 17.7 Write `docs/deployment.md`:
...
- [ ] 17.8 Write `docs/secrets.md`:
  - List every secret with owner + rotation cadence + where it lives
  - Rotation procedure for each
- [ ] 17.9 Add Makefile targets `deploy-functions PROJECT_REF=...` (calls `supabase functions deploy ${FUNCTIONS} --project-ref $$PROJECT_REF`) and `db-diff` (generates a candidate migration from local schema diff against preview)
- [ ] 17.10 Test: Open a PR touching `marketing/public/index.html` and verify CI passes; merge to main and verify the marketing deploy workflow runs (in dry-run mode if infra repo not yet wired)
- [ ] 17.11 Test: Open a PR touching `supabase/migrations/`, verify CI db-lint passes; merge and verify supabase deploy workflow runs preview then prod
- [ ] 17.12 Manual run: `gh workflow run build-mobile.yml -f profile=preview -f platform=ios` and verify EAS build queued
