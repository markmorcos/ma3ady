...
- [x] 1.1 `make migrate-new NAME=status_transitions` — create `validate_status_transition(...)` function and a CHECK constraint via trigger that calls it on UPDATE
- [x] 1.2 Modify `update-appointment-status` Edge Function to call `validate_status_transition` before updating; map invalid transitions to HTTP 422 with code `invalid_transition`
- [x] 1.3 Write Edge Function `supabase/functions/reschedule-appointment/index.ts`:
...
- [x] 1.4 Modify `manage-appointment` Edge Function (the `implement-public-booking-flow` change) to delegate to `reschedule-appointment` for the reschedule action — single source of truth for the logic
- [x] 1.5 Write `app/(app)/bookings/[id].tsx`:
...
- [x] 1.6 Write `app/(app)/bookings/[id]/reschedule.tsx`:
