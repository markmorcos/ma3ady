...
- **ADDED** route group `app/(admin)/`:
  ...
  - `(tabs)/team.tsx` — team members + invite (uses `invite-member` from change 09)
...
- **ADDED** Edge Function `update-appointment-status/`:
  ...
  - Validates state transition (defined in change 13's state machine)
...
- Sets the stage for change 12 (rules grid editor inside admin) and change 13 (reschedule/cancel which reuses the status state machine).
