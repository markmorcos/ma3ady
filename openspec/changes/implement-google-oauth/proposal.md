...
- **ADDED** `auth.ma3ady.com/callback` route (lives in the tenant-landing app from the `setup-tenant-landing-app` change; in dev, Expo proxy URL is used)
...
- Required by the `implement-tenant-onboarding`, `implement-admin-mobile-dashboard` and `implement-reschedule-and-cancel` changes.
- Anonymous booking flow (the `implement-public-booking-flow` change) does not depend on this — guests never sign in. But the "claim prior bookings" feature does.
