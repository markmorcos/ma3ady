...
8. **Display follows `project.md` §10**. The function returns `timestamptz` (UTC). The mobile app passes those through `useDisplayTimezone()` which renders in the tenant TZ by default on the public booking flow, and offers a session-only toggle to device TZ. Rules and exceptions in the editor render in tenant TZ exclusively (admins are configuring "the business's hours," not theirs).
