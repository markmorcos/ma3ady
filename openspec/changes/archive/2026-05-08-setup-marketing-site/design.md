...
- Per-tenant marketing pages (the *tenant landing app* in the `setup-tenant-landing-app` change covers that).
...
8. **CTA points at the demo tenant landing page** (the `setup-tenant-landing-app` change) for now. Once the app is published, the CTA flips to App Store / Play Store badges.
