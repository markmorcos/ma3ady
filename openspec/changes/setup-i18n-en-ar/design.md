...
6. **`start`/`end` only, no `left`/`right` in styles** — enforced by an ESLint rule added in the `setup-design-system` change (design system). RTL correctness depends on this discipline.
