/**
 * Server-side Material Design 3 palette generator for tenant-landing.
 *
 * Mirrors `src/design/palette.ts` in the mobile app: an OKLCH lightness-ramp
 * approximation of HCT. Outputs the full M3 role table for a source hex in
 * the requested mode. The result is consumed by the tenant page layout to
 * inject per-tenant CSS variables into the document head.
 */
...
/** kebab-case the camelCase role name for CSS variable injection. */
function kebab(s: string): string {
  return s.replace(/[A-Z]/g, (m) => `-${m.toLowerCase()}`);
}

/**
 * Produce a CSS variable declaration string for both light and dark modes.
 * Caller injects this into a `<style>` tag in the page head.
 *
 * Each role becomes `--{kebab-role}: <hex>;`. The dark-mode counterpart is
 * scoped under `@media (prefers-color-scheme: dark)`.
 */
export function paletteCss(sourceHex: string): string {
  const light = buildPalette(sourceHex, 'light');
  const dark = buildPalette(sourceHex, 'dark');
  const toBlock = (roles: M3Roles): string =>
    Object.entries(roles)
      .map(([k, v]) => `--${kebab(k)}:${v};`)
      .join('');
  return `:root{${toBlock(light)}}@media (prefers-color-scheme:dark){:root{${toBlock(dark)}}}`;
}
